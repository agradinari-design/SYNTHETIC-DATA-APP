...
if page == "Data Generation":
    ...
    if st.button("Generate", type="primary"):
        if not uploaded_file or not parsed_schema:
            ...
            with st.spinner("Generating data and populating database..."):
                try:
                    ...
                    if isinstance(raw_result, dict):
                        ordered_tables = [t for t in table_list if t in raw_result]
                        for remaining in raw_result.keys():
                            if remaining not in ordered_tables:
                                ordered_tables.append(remaining)

                        for table_name in ordered_tables:
                            data = raw_result[table_name]
